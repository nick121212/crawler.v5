...
export class DiscoverLinks {
     ...
     */
    constructor(private settings: DiscoverSettings) {
     ...
     * @returns {boolean}
     */
    public protocolSupported(url: string): boolean {
        let protocol: string,
            { allowedProtocols = [] } = this.settings;
     ...
     * @returns boolean
     */
    public extendSupported(suffix: string): boolean {
        return !suffixs.some((value: string) => {
     ...
     * @returns boolean
     */
    public pathSupported(urlPath: string): boolean {
        let res = false,
            { whitePathList } = this.settings;
     ...
     * @returns {Array}
     */
    public cleanExpandResources(urlMatch: any = [], queueItem: IQueueItem = { url: "" }): any {
     ...
     * @returns {*|string}
     */
    public cleanURL(queueItem: IQueueItem, url: any): string {
     ...
     * @returns {boolean|*}
     */
    public depthAllowed(queueItem: IQueueItem): boolean {
     ...
     * @returns {boolean}
     */
    public urlIsAllowed(parsedURL: string | any): boolean {
     ...
     * @returns {*}
     */
    public discoverResources(queueItem: IQueueItem = {}): Array<any> {
